...
| «Задача в программе маленькая — соберу без манифеста» | Строка файла программы и есть требование: без дословной цитаты мини-прогон везёт твою интерпретацию строки, а не строку. |
| «Проверю сам, я же это строил» | Строивший не проверяет: он видит не результат, а своё намерение. Слепая приёмка слепая — субагент, бриф, без спеки. |
| «Инцидент горячий — фикс без красного теста» | Горячий фикс без теста — это тот же инцидент, назначенный на завтра. Механизм без красного теста не починен, а спрятан. |
| «Доки напишу по спецификации, так быстрее» | Спецификация описывает намерение, доки описывают код. Написанное по спеке уверенно описывает проект, которого нет. |
| «Откачусь сразу, потом разберёмся» | Откат стирает улики вместе с поломкой. Снимок доказательств до отката — или механизма больше не существует. |
