...
**`serial` execution is the one exception, and it is a dial, not a failure.** When `execution` is `serial` in `state.js` — asked for at the start (`phases/0-modes.md`) or switched on mid-run when the provider starts dropping parallel sessions — the wave still defines the frontier, but launches go out one at a time: the next ticket leaves when the previous one returns, and the per-wave state write of «Before each ticket» becomes per-ticket. Everything above is about not wasting parallelism the harness has; this is about not depending on parallelism it does not have. Zones, blockers, «nothing parallelises with ticket 01» — all hold unchanged.
